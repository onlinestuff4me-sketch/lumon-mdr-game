...
- groups move outwards across orientation: measured mean offset from center
...
|---|---|
| gesture lifecycle | A tap-lift releases the gesture and the packet is still draggable to the bin. Five terminating paths — tap on empty board, zero-length drag, empty box, drag off the top edge, canceled pointer — each release the gesture **and** leave input live, proven by completing a real lift afterwards. |
...
| bin catch | A packet dropped 40px above a bin's top edge still lands in it; one dropped far above stays in hand. The catch zone exists because the packet's center is what is tested and the box is a hundred pixels wide — demanding the center fully inside the bin meant drops released at its top edge fell back into the hand. |
